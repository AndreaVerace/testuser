...
public class TestUser {
	...
	private static void testFindByExample(UserService userService) throws Exception {
		...
		List<User> risultatifindByExample = userService.findByExample(new User("MA"));
	...
	private static void testCercaTuttiQuelliCheUsernameIniziaCon(UserService userService) throws Exception {
		...
		if(userService.listAll().size() < 1)
			throw new Exception("non ci sono elementi presenti.");
		
		String iniziali = "g.";
		
		List<User> result = userService.cercaTuttiQuelliCheUsernameIniziaCon(iniziali);
		
		if(result.size() < 1)
			throw new Exception("FALLITO.");
	...
	private static void testCercaTuttiQuelliCreatiPrimaDi(UserService userService) throws Exception {
		...
		Date data = new SimpleDateFormat("dd-MM-yyyy").parse("01-01-2022");
	...
	private static void testCercaPerCognomeENomeCheInziaCon(UserService userService) throws Exception {
		...
		String cognome = "PARISI";
		String nome = "G";
		
		List<User> result = userService.cercaPerCognomeENomeCheInziaCon(cognome, nome);
	...
	private static void testAccedi(UserService userService) throws Exception {
		...
		String login = "luigi89@live.it";
		String password = "luigi";
		
		User result = userService.accedi(login, password);
